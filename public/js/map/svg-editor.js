        ...
        if (this.attributes) {
            ...
            for (var attr in this.attributes){
				...
				if(attr != 'd'){
					...
					row.append($('<td><input type="text" class="attribute-editor textData" id="'+this.id+'|'+attr+'" name="'+this.id+'['+attr+']" value="' + value + '"/></td>'));
				}
                
                table.append(row); 
            }
            var row = $('<tr></tr>');
            row.append('<td></td><td><input type="submit" name="save" class="saveData" value="Save Property"/></td>');
            table.append(row);
...
$(document).ready(function(){
    $('body').on('click', '.saveData', function(){
        ...
        $('.textData').each(function(index){
            ...
            elem = elem.split('|');
            if(elem[1] != 'id'){
                svg_tree.id_to_element[elem[0]].attributes[elem[1]] = $(this).val();
            }else{
                idElem = elem[0];
                idVal = $(this).val();
            }
        }).promise().done(function(){
            svg_tree.id_to_element[idElem].attributes['id'] = idVal;
