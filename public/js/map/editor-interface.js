...
$(document).ready(function() {   
    ...
    $('body').on('click','#closeEditor', function(){
        console.log(svg_tree);
        /*$.ajax({
            type:'POST',
            url: route()+'/map/saveSVG',
            data: {'_token':$('input[name=csrf]').val(),'id': dataID,'svg':svg_tree.root.toString()},
            success: function(){
                $('#mapEditor').html('');
                $('.overlay').fadeOut(200);
                $('#mapEditor').fadeOut(200);
            }
        });*/
    });
	
    
	// $('.attribute-editor').on('blur', function(e) {
 //        alert(1213);
 //        console.log("==========");
